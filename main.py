...
def search_order(product_name):
    try:
        ...
        for current_order in root.findall('order'):
            ...
            # Check if the product name is in the order
            product_found = False
            for product in current_order.find('products'):
                if product.find('product_name').text.lower() == product_name.lower():
                    product_found = True
                    product_id = product.get('id')
                    product_category = product.find('category').text
                    product_name = product.find('product_name').text
        ...
        if output == "":
            output = f"No order found for product: {product_name}\n"
        ...
        text_area.insert(tk.END, "Error parsing XML content.\n")
        text_area.see(tk.END)
...
def search_order_json(product_name):
    try:
        data = json.loads(content)
        orders = data['supply_stream']['order']
        ...
        text_area.insert(tk.END, "Error parsing JSON content.\n")
        text_area.see(tk.END)
...
# Entry box for product name
entry_product = tk.Entry(frame_search, width=30)
entry_product.pack(side=tk.LEFT, padx=10)

# Button to search for order by product name
button_search = tk.Button(frame_search, text="Search XML Order by Product",
                          command=lambda: search_order(entry_product.get()))
button_search.pack(side=tk.LEFT, padx=10)

button_search_json = tk.Button(frame_search, text="Search JSON Order by Product",
                               command=lambda: search_order_json(entry_product.get()))
button_search_json.pack(side=tk.LEFT, padx=10)
